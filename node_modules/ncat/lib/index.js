...
const concat = new _concatWithSourcemaps2.default(argv.output !== undefined && argv.output !== null && argv.map, argv.output ? _path2.default.basename(argv.output) : '', '\n');
/**
...
function handleFile(file) {
	if (argv.map && argv.output) {
		return _fsExtra2.default.readFile(file).then(content => (0, _pify2.default)(_sourceMapResolve2.default.resolveSourceMap)(content.toString(), file, _fsExtra2.default.readFile).then(map => {
			...
			console.log(chalk.bold.yellow(`The sourcemap ${err.path} referenced in ${file} cannot be read and will be ignored`));
		}).then(result => {
			if (!result) {
				log('add', file);
				return {
